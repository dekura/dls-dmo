...
class UNet16(nn.Module):
    def __init__(self, num_classes=1, num_filters=32, pretrained=True, upp_scale = 1):
        """
        :param num_classes:
        :param num_filters:
        :param pretrained:
        ...
        self.pool = nn.MaxPool2d(2, 2)
        self.pool0_1 = PoolConv(nb_filters[0])
        self.pool1_2 = PoolConv(nb_filters[1])
        self.pool2_3 = PoolConv(nb_filters[2])
        self.pool3_4 = PoolConv(nb_filters[3])
        self.pool4_5 = PoolConv(nb_filters[4])
    ...
    def forward(self, x):
        conv1 = self.conv1(x)
        conv2 = self.conv2(self.pool0_1(conv1))
        conv3 = self.conv3(self.pool1_2(conv2))
        conv4 = self.conv4(self.pool2_3(conv3))
        conv5 = self.conv5(self.pool3_4(conv4))

        center = self.center(self.pool4_5(conv5))
        ...
        if self.num_classes > 1:
            ...
            x_out = self.tanh(self.final(dec1))
