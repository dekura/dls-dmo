...
class UNet(nn.Module):
    def __init__(self, input_nc=1, output_nc=1):
        ...
        self.conv3_1 = VGGBlock(nb_filter[3]+nb_filter[2], nb_filter[3], nb_filter[3])
        self.conv2_2 = VGGBlock(nb_filter[2]+nb_filter[1], nb_filter[2], nb_filter[2])
        self.conv1_3 = VGGBlock(nb_filter[1]+nb_filter[0], nb_filter[1], nb_filter[1])
        self.conv0_4 = VGGBlock(nb_filter[0]+int(nb_filter[0]/2), nb_filter[0], nb_filter[0])
...
class UpConv(nn.Module):
    def __init__(self, channels):
        ...
        self.up_conv = nn.PixelShuffle(2)
    def forward(self, input):
...
class NestedUNet(nn.Module):
    def __init__(self, input_nc, output_nc=1, lambda_o=1, tanh_act=True, deepsupervision=True, upp_scale=2):
        ...
        self.conv0_1 = VGGBlock(nb_filter[0]+int(nb_filter[0]/2), nb_filter[0], nb_filter[0])
        self.conv1_1 = VGGBlock(nb_filter[1]+nb_filter[0], nb_filter[1], nb_filter[1])
        self.up1_1 = UpConv(nb_filter[1])
        self.conv2_1 = VGGBlock(nb_filter[2]+nb_filter[1], nb_filter[2], nb_filter[2])
        self.up2_1 = UpConv(nb_filter[2])
        self.conv3_1 = VGGBlock(nb_filter[3]+nb_filter[2], nb_filter[3], nb_filter[3])
        self.up3_1 = UpConv(nb_filter[3])

        self.conv0_2 = VGGBlock(nb_filter[0]*2+int(nb_filter[0]/2), nb_filter[0], nb_filter[0])
        self.conv1_2 = VGGBlock(nb_filter[1]*2+nb_filter[0], nb_filter[1], nb_filter[1])
        self.up1_2 = UpConv(nb_filter[1])
        self.conv2_2 = VGGBlock(nb_filter[2]*2+nb_filter[1], nb_filter[2], nb_filter[2])
        self.up2_2 = UpConv(nb_filter[2])

        self.conv0_3 = VGGBlock(nb_filter[0]*3+int(nb_filter[0]/2), nb_filter[0], nb_filter[0])
        self.conv1_3 = VGGBlock(nb_filter[1]*3+nb_filter[0], nb_filter[1], nb_filter[1])
        self.up1_3 = UpConv(nb_filter[1])

        self.conv0_4 = VGGBlock(nb_filter[0]*4+int(nb_filter[0]/2), nb_filter[0], nb_filter[0])
    ...
    def forward(self, input):
        ...
        x0_0 = self.conv0_0(input)
        x1_0 = self.conv1_0(self.pool0_0(x0_0))
        # print('x_00 shape:',x0_0.size())
        # print('self.up1_0(x1_0) shape: ', self.up1_0(x1_0).size())
